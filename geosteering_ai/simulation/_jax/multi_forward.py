...
def simulate_multi_jax_batched_grouped(
    ...
    *,
    ...
    # Reassembly VETORIZADO (v2.49 — dívida do Sprint C): pré-aloca o tensor de saída
    # UMA vez e espalha cada grupo por indexação avançada `H_tensor[sel] = Hg`. `sel`
    # mapeia índice local→global, então o scatter reposiciona cada grupo na ORDEM
    # ORIGINAL dos modelos. Substitui o padrão antigo (lista `[None]*n_models` +
    # atribuição modelo-a-modelo + `np.stack`).
    #
    # GANHO REAL = PICO DE MEMÓRIA (~2× a 30k×600), não wall-time: medido isolado, a
    # cópia em si é ~equivalente (memcpy-bound, ~1 s a 30k; `res.H_tensor` já é numpy
    # — converte em forward.py:531/1159). O padrão antigo retinha VIEWS de TODOS os K
    # grupos vivos até o `np.stack` final → pico ≈ 2× a saída (4995 MB medido); o
    # scatter copia e LIBERA cada grupo na iteração seguinte → pico ≈ 1× a saída
    # (2532 MB medido, −49%). Isso evita o OOM que matava o run de 30k para tensores
    # multi-config (nTR·nAng·nf > 1). O custo dominante de ESCALA continua sendo o
    # nº de GRUPOS (overhead de launch JAX por grupo) — amostre poucas geometrias.
