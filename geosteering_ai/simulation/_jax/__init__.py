...
def _setup_xla_environment() -> None:
    ...
    # Sprint v2.51 (warmup): default ESTÁVEL em ~/.cache (sobrevive reboot →
    # cache-hits XLA cross-run REAIS), em vez de /tmp (efêmero). Prioridade:
...
try:
    ...
    # Sprint v2.51 (warmup) — persiste TODO compile (mesmo rápido) no cache de
    # disco. Default do JAX é ~1.0s → compiles de bucket abaixo do limiar NÃO
    # persistiam, anulando o cache cross-run. 0.0 = persiste tudo. Version-guarded
