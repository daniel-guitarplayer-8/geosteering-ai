# -*- coding: utf-8 -*-
# mypy: ignore-errors
# ^ Adaptador de backend: chama APIs NATIVAS (matplotlib/pyqtgraph/plotly/vispy)
#   sobre o ``SubplotHandle`` (Protocol opaco) — tipagem estrita destes adaptadores
#   é DEFERIDA (a ABC ``base.py`` PERMANECE tipada). Preserva o estado de origem
#   (o pacote era excluído do mypy em ``simulation/tests/``; spec 0006/0011).
# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║  geosteering_ai/gui/plot_backends/plotly_canvas.py                        ║
# ║  ---------------------------------------------------------------------    ║
...
class PlotlyCanvas(PlotCanvas):
    ...
    Note:
        Requer Qt WebEngine. PyQt6: ``pip install PyQt6-WebEngine``;
        PySide6: já incluso. O ``QWebEngineView`` é resolvido pelo binding
        ATIVO via :func:`geosteering_ai.gui.qt_compat.load_qwebengineview`
        (portátil PyQt6/PySide6 — sem import hardcoded).
    """
    ...
    def __init__(
        ...
        try:
            qwebengineview = load_qwebengineview()  # binding ativo (PyQt6/PySide6)
        except ImportError as exc:
            raise ImportError(
                "Backend Plotly requer Qt WebEngine. "
                "PyQt6: pip install PyQt6-WebEngine | PySide6: já incluso."
            ) from exc
