# -*- coding: utf-8 -*-
# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║  geosteering_ai/gui/persistence/snapshot.py                               ║
# ║  ---------------------------------------------------------------------    ║
# ║  Projeto     : Geosteering AI v2.0                                        ║
# ║  Subsistema  : GUI — Persistência Assíncrona de Experimento               ║
# ║  Autor       : Daniel Leal                                                ║
# ║  Criação     : 2026-04-29 (v2.11)                                         ║
# ║  Status      : Produção                                                   ║
# ║  Framework   : QThread + json.dumps (stdlib)                              ║
# ║  Dependências: gui.qt_compat, gui.persistence.atomic                      ║
# ║  ---------------------------------------------------------------------    ║
# ║  FINALIDADE                                                               ║
...
from geosteering_ai.gui.qt_compat import QThread, Signal
...
class SnapshotPersistThread(QThread):
    ...
    def run(self) -> None:  # noqa: D401
        ...
        """
        try:
            # Escrita ATÔMICA (write-temp → fsync → os.replace) — crash-safe.
            # Endurecimento spec 0007: substitui o ``open(path,"w").write()`` direto
            # (que podia deixar um .session truncado num crash). ``atomic_write_text``
            # cria o diretório pai e garante atomicidade no mesmo filesystem.
            atomic_write_text(self._target_path, self._json_text)
