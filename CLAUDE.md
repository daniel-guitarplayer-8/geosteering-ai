...
| **Simulation Manager** | **v2.23 (2026-05-10) — Fastmath Dual-Mode + Threads Adaptativos** (multi-agente): A.1 `@njit(fastmath=True)` em `_numba/geometry.py` (3 funcs) + `_numba/rotation.py` (2 funcs) — paridade Fortran <1e-12 PRESERVADA em 13/13 modelos canônicos. A.2 auto-detect via `recommend_default_parallelism()` no `__post_init__` quando ambos `None` — backward-compat preservada (`n_workers=1` = single-process). Defesa em camadas KB-019 (warning >4× phys_cores). Hook `setup-environment.sh` exibe topologia + defaults v2.23. 7 novos testes + 1 atualizado. Reviewers paralelos (physics + perf + code) + 3 fixes pre-merge. v2.22.7-docs: pré-mortem inaugural + §74 + §75 + §24.4 + skill `geosteering-premortem-analyst`. v2.22.6: Fase 1 Fundação Multi-Agente COMPLETA (§22.1). v2.22.4: `use_flat_prange=True` default. v2.22: FLAT prange 4D. v2.21: causa-raiz KB-013 Cenário E 46k→122k. Paridade Fortran <1e-12 inviolável · 1624 PASS / 295 SKIP / 0 FAIL na suite total (v2.23) |
