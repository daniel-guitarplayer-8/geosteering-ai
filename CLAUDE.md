...
| **Simulation Manager** | **v2.30 (2026-05-11) — CLI Multi-Dimensional (multi-freq + multi-dip + multi-TR)**: personalização completa do CLI Geosteering AI. Novos argumentos em `simulate` subcomando: `--frequencies HZ`, `--dips DEG`, `--tr-spacings M` (CSV floats, defaults preservados). Novos argumentos em `benchmark` subcomando: mesmos 3, agora como **overrides** de cenário. Novo **Cenário G** (máxima combinatória: 4 freq × 4 TR × 4 dips = 256 combos por modelo). Helper `_parse_float_list()` com edge cases robustos (vazio, ",,," → default, ValueError → log). Logs atualizados: "X freq, Y dips, Z TR". **Arquitetura**: lazy imports preservados (handlers importados apenas quando subcomando invocado), 100% backward-compatible (novos args opcionais). **Testes**: +8 novos (help, parsing, multi-dim smoke), 25/25 CLI PASS, 10/10 Fortran parity PASS, 42/42 total. **CodeRabbit**: 0 findings (após fix 2 menores _parse_float_list). **Paridade Fortran**: <1e-12 preservada. Relatório: `docs/reports/v2.30_2026-05-11.md`. v2.29.3 (2026-05-11): Investigação regressão — CONFIRMADO NÃO HÁ REGRESSÃO (v2.29.2 +3.8% melhor). Infraestrutura anti-regressão: hook `check-perf-regression.sh`, baseline doc, skill reviewer. 37/37 PASS. v2.29.2: Cache LRU configurável (default_max_bytes 10% RAM), UI QSettings, signal reinstancia preservando snapshots. v2.29.1: Fix NUMBA_NUM_THREADS no PAI (170k mod/h atingido). v2.29: Back to Basics — pool efêmero + warmup inline. Paridade Fortran <1e-12 inviolável · 1660+ PASS / 295 SKIP / 0 FAIL na suite total |
