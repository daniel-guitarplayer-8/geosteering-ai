...
| **Simulation Manager** | **v2.24 (2026-05-10) — Débitos Técnicos + I2.5 + I2.6** (multi-agente): 1.1 `scripts/count_pytest_pass.py` automatiza contagem PASS/SKIP/FAIL (elimina M2 recorrente). 1.2 `setup-environment.sh` totalmente em PT-BR acentuado. 1.3 `cache=True` explícito em TODOS os 13 decoradores `@njit` em `_numba/` (propagation 2 + dipoles 2 + rotation 2 + hankel 4 + geometry 3). 1.4 `use_fastmath` documentado como status documental — dispatcher real deferido para v2.25+ (substituído por estratégia superior `fastmath` seletivo + pairwise summation — ver Sprint v2.25 planejado). **I2.5**: hooks `check-ptbr-accentuation.sh` (catálogo `.claude/ptbr-words.txt` com 60+ pares, registrado em settings.json PostToolUse) + `generate-pr-description.sh` (template `.claude/templates/pr_description_template.md`). **I2.6**: CLI MVP `geosteering-cli` (`simulate`/`benchmark`/`version`) em `geosteering_ai/cli/` + entry point `pyproject.toml`. 41 novos testes (11 v2.24 + 17 I2.5 + 13 CLI). Paridade Fortran <1e-12 PRESERVADA em 7 modelos canônicos (10/10 testes em `test_simulation_compare_fortran.py`). v2.23: Fastmath Dual-Mode + Threads Adaptativos. v2.22.7-docs: pré-mortem inaugural + §74 + §75 + §24.4. v2.22.6: Fase 1 Fundação Multi-Agente COMPLETA (§22.1). v2.22.4: `use_flat_prange=True` default. v2.21: causa-raiz KB-013 Cenário E 46k→122k. Paridade Fortran <1e-12 inviolável · 1665+ PASS / 295 SKIP / 0 FAIL na suite total (v2.24) |
