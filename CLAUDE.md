...
| **Simulation Manager** | **v2.28 (2026-05-11) — Fix warmup incompleto: Warmups C+D para cobertura total**: v2.27 eliminou saturação LLVM mas introduziu regressão de throughput (~55k mod/h vs 800k–1.4M esperado). Análise de timestamps `__pycache__` revelou 5 funções JIT (`propagation.common_arrays`, `propagation.common_factors`, `geometry.find_layers_tr`, `geometry._sanitize_profile_kernel`, `kernel.precompute_common_arrays_cache`) compiladas APÓS `t0_sim`. Causa: warmup A+B usava `rho_h==rho_v` (isotrópico) + `dip=0°` — não acionava especializações reais. Workers 1..N compilavam essas funções INLINE no warmup secundário após `t0_sim` → ~30s contados como tempo de simulação. Fix: Warmup C (anisotrópico `_rho_v = _rho * 0.3` + dip=0°) + Warmup D (anisotrópico + dip=30°) em `_run_numba_warmup_task`. Paridade Fortran <1e-12 PRESERVADA. 2 novos testes (total 12 warmup PASS). v2.27: 1 future ao invés de n_workers (elimina saturação LLVM). v2.26: warmup movido do inicializador para Future cancelável (elimina hang no shutdown). v2.25: fix shape warmup. v2.24: Débitos Técnicos + I2.5 + I2.6. v2.23: Fastmath + Threads Adaptativos. v2.22.7-docs: pré-mortem + §74 + §75 + §24.4. v2.22.6: Fase 1 Fundação Multi-Agente COMPLETA. v2.22.4: `use_flat_prange=True` default. v2.21: causa-raiz KB-013 Cenário E 46k→122k. Paridade Fortran <1e-12 inviolável · 1669+ PASS / 295 SKIP / 0 FAIL na suite total (v2.28) |
