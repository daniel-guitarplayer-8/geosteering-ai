...
| **Simulation Manager** | **v2.35 (2026-05-15) — Bundle de Estabilização & Observabilidade (3 sprints)**: **v2.33 — pytest-qt para GUI**: suite com 16 testes cobrindo ~10.7k linhas Qt antes sem cobertura (compat Qt, SimRequest dataclass, SimulationThread sinais + pause/resume cooperativo, widgets leves). `tests/conftest.py` raiz alinha QT_API com sm_qt_compat (evita cross-binding TypeError). Fixtures `qt_binding` + `mock_simulation_thread` + `mock_sim_request` em `tests/conftest_qt.py`. CI usa `xvfb-run -a pytest` headless. `pytest-qt>=4.4` adicionado a `[dev]`. Marker `gui`. **v2.34 — `geosteering-warmup` no CI/CD**: workflow Actions adiciona steps `geosteering-warmup --verbose` + `geosteering-cli benchmark --scenario E --n 200` antes do pytest. Nova métrica `E_n200_warm` em `.claude/perf_baseline.json` (placeholder a ser medido pós-merge); seção "Warm-Cache Baseline (CI)" em `docs/PERFORMANCE_BASELINE.md`. `continue-on-error: true` no benchmark (observabilidade, não gating). **v2.35 — Cenário H**: stress-test multi-core 8×8×8 = 512 combos (8 freq log-spaced 1 kHz–200 kHz × 8 TRs 0.25–2.5 m × 8 dips 0°–105°). 3 novos testes (help, dict shape, smoke `--n 2 --workers 2 --threads 2` em <600s). **Validação**: 16/16 GUI + 35/35 fast + 10/10 Fortran parity <1e-12 PASS. **Sem mudança no caminho crítico Numba JIT** — paridade Fortran sagrada preservada. v2.32 (2026-05-13) — `geosteering-warmup` Entry Point + Dívidas v2.31: novo comando standalone para aquecer Numba+LLVM em CI/notebooks antes de simular. Refator `_warmup_numba_tier2_sync()` extraído em `cli/main.py`. v2.31 (2026-05-12) — Otimização Warmup JIT (Parte 1): aplica correções da Parte 1 das estratégias de warmup do relatório `warmup_analysis_jit_2026-05-12.md`. (1) **Anomalia `hmd_tiv`/`vmd` 2 `.nbc` eliminada**: helper `_to_writeable()` em `_jax/kernel.py::_dipoles_numba_host` força arrays writeable=True antes de chamar Numba via `pure_callback`. Resultado: 1 `.nbc` por função em vez de 2 (`hmd_tiv` 2.6→1.275 MB; `vmd` 1.47 MB→683 KB; redução total ~2 MB; estimativa −20-40 s no cold-start). (2) **NUMBA_CACHE_DIR em tmpfs**: `cli/main.py` define cache em `$TMPDIR/geosteering_numba_cache` (permissões 0o700) antes de imports pesados — reduz I/O do carregamento de `.nbc` em chamadas subsequentes (estimativa −10-30 s). (3) **5 guard tests novos**: `tests/test_simulation_numba_specializations.py` garante 1 especialização por função + 1 `.nbc` em disco + NUMBA_CACHE_DIR configurado. **CodeRabbit**: 0 findings após fix 1 major (permissões restritivas). **Paridade Fortran**: <1e-12 preservada (10/10 PASS). **Throughput baseline**: 100% preservado. **Suite**: 56/56 PASS em 37s. Relatório: `docs/reports/v2.31_warmup_optimization_2026-05-12.md`. v2.30 (2026-05-11): CLI Multi-Dim (--frequencies/--dips/--tr-spacings + Cenário G). v2.29.3: Investigação regressão CONFIRMADO NÃO HÁ REGRESSÃO + infraestrutura anti-regressão. v2.29.2: Cache LRU configurável. v2.29.1: Fix NUMBA_NUM_THREADS no PAI. v2.29: Back to Basics — pool efêmero. Paridade Fortran <1e-12 inviolável · 1665+ PASS / 295 SKIP / 0 FAIL na suite total |
