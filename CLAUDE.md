...
| **Simulation Manager** | **v2.29.3 (2026-05-11) — Investigação de regressão + infraestrutura anti-regressão**: usuário reportou regressão pós-v2.29.2 em Cenário E n=2000. Benchmarks lado-a-lado (commit `24fba72` v2.29.1 vs `e3a0617` v2.29.2, cache JIT limpo, 3 runs cada) CONFIRMAM **NÃO HÁ REGRESSÃO** — v2.29.2 é +3.8% melhor (mediana 95,124 vs 91,620 mod/h). Análise estática (2 Explore agents) confirma: todas as 4 mudanças v2.29.2 são periféricas ao caminho crítico Numba JIT. **Mudanças**: (1) Fix `is_paused()` em `_resume_simulation` (era `sim.is_paused` sem parênteses; não afetava throughput por idempotência). (2) Hook `.claude/hooks/check-perf-regression.sh` anti-regressão (Cenário E n=200 vs `.claude/perf_baseline.json` com threshold 90% e severidade INALTERADO/MELHORIA/MINOR/MAJOR/CRÍTICA). (3) `docs/PERFORMANCE_BASELINE.md` tabela canônica + processo de atualização (3 runs + mediana). (4) Skill `.claude/commands/geosteering-perf-baseline.md` reviewer perf-aware. **Arquitetura anti-regressão**: 5 níveis de defesa (hooks + baseline doc + skills + 37 tests regressão + processo). Suite 37/37 PASS + smoke 0 falhas + paridade Fortran <1e-12 PRESERVADA. CodeRabbit: 6 findings (4 hook + 2 skill) → todos aplicados. **Respostas conceituais ao usuário**: workers em duas camadas (`sm_workers.py` Qt GUI + `_workers.py` core); CLI via `pip install -e .` + `geosteering-cli {simulate,benchmark,version}`; refactor MVC do SM viável Sprint v3.0+ após pytest-qt golden path (v2.27); 5 níveis anti-regressão. Limitação: 150k mod/h não atingido em medições atuais (~95k em n=2000); investigar Sprint v2.30+. **v2.29.2 (2026-05-11) — Cache LRU configurável + varredura GUI**: resolve feature pendente desde v2.5 (a mensagem do diálogo prometia "Aumente o limite de cache em Preferências (futuro)"). `LRUPlotCache` tinha limite hardcoded 500 MB em `sm_plot_cache.py:73` e `simulation_manager.py:7368`. Tensores multi-freq × multi-angle `complex128` (1000 mod × 2 TR × 4 dips × 600 pos × 4 freq × 9 comp × 16 B = **2.77 GB**) excediam o limite em 5.5×. **Solução**: `default_max_bytes()` em `sm_plot_cache.py` (10% RAM via psutil, piso 500 MB, teto 4 GB, fallback robusto em qualquer Exception); `LRUPlotCache.__init__(max_bytes: Optional[float] = None)` ativa auto-detect; QSettings (`cache/max_bytes_mb`, `cache/maxlen`) lidos em `MainWindow.__init__` com validação; UI Preferências group "Cache LRU de Tensores" com 2× QSpinBox + botão Auto-detect; signal `cache_settings_changed(int, int)` reinstancia cache preservando snapshots e atualiza widgets dependentes (`_update_cache_status`, `_refresh_results_experiment_combo`, `mark_history_out_of_cache`); mensagem do diálogo agora dinâmica com limite atual + link "Preferências → Cache LRU". **Varredura GUI completa**: smoke test reportava 3 falhas pré-existentes desde v2.29 (`is_paused`/`is_cancelled` viraram métodos em "Back to Basics" mas smoke test tratava como properties) — corrigidas. Confirmado que GUI v2.21 está integralmente adaptada (12 classes preservadas) — nada para adaptar. 7 novos testes LRU PASS + 30 regressão PASS + smoke test 0 falhas. Paridade Fortran <1e-12 PRESERVADA. CodeRabbit: 4 findings na 1ª passada (exception coverage, byte eviction test, monkeypatch cleanup, GUI lifecycle hooks) → todos corrigidos → 0 findings na 2ª passada. **v2.29.1 (2026-05-11) — Fix `NUMBA_NUM_THREADS` no PAI + restauração de 150k mod/h**: a v2.29 reintroduziu bug latente — setar `os.environ["NUMBA_NUM_THREADS"]` dentro de `run_numba_chunk` (worker) falha em modo spawn quando o ambiente do pai tem env var distinta. Cadeia de import durante o bootstrap do worker carrega numba ANTES de qualquer linha de `run_numba_chunk` rodar → `RuntimeError: Cannot set NUMBA_NUM_THREADS to a different value`. Fix: mover setup do env var para `SimulationThread.run` e `_run_numba_parallel_sync` (PAI) com `try/finally` que restaura estado original (evita vazamento). Workers spawned herdam env var correto; `multi_forward.py:1010-1028` chama `numba.set_num_threads(req.n_threads)` para mascarar threads ativas. **Throughput 149,984 mod/h ≈ 150k** atingido em reprodutor empírico (cenário do bug: 2 freqs × 2 dips × 1 TR + 4w×4t + NUMBA_NUM_THREADS=2 herdado do shell). CLI single-process: Cenário E 145k mod/h, Cenário A 3.15M mod/h. +3 guard tests v2.29.1 (worker NÃO seta env var; pai SETA antes do pool; try/finally restaura). Paridade Fortran <1e-12 PRESERVADA em 10/10 testes canônicos. CodeRabbit: 0 findings. **v2.29 (2026-05-11) — Back to Basics: reversão arquitetural definitiva**: após 4 tentativas (v2.25–v2.28) sem resolver regressão (throughput 75–107k vs >150k esperado, warmup visível ~33s, Python travando ao fechar), análise comparativa com `old_geosteering_ai/simulation/tests/sm_workers.py` (869 LOC, comprovadamente funcional) revelou que a infra v2.18+ (pool persistente + PoolWarmupThread + `_run_numba_warmup_task` + `_WORKER_INITIALIZED` + `t0_sim` + NOOPs) é **arquiteturalmente incompatível** com cold-start JIT do Numba. Reverteu para arquitetura OLD: **pool EFÊMERO** (`with ProcessPoolExecutor`), **warmup INLINE** em `run_numba_chunk` com dados REAIS de `chunk[0]` (cobre TODOS paths Numba automaticamente), `t0 = time.perf_counter()` único antes do pool. Removidos: `_PERSISTENT_POOL`, `_acquire_numba_pool`, `release_numba_pool`, `PoolWarmupThread`, `_run_numba_warmup_task`, `_numba_init_worker`, `_noop`, `_WORKER_INITIALIZED`, `t0_sim`, `_prewarm_numba_pool` (MainWindow), `lbl_warmup_status`, `_warmup_thread`, 4 métodos GUI `_start_background_warmup`/`_on_warmup_*`/`_on_backend_changed_warmup`. Preservado: NumbaPrimer (v2.9), pause/cancel cooperativo (v2.11), `NUMBA_NUM_THREADS` setup (v2.16), `detect_cpu_topology` log (v2.17). sm_workers.py: **1478→1062 LOC (-416)**. Paridade Fortran <1e-12 PRESERVADA em 10/10 testes canônicos. Hang ao fechar **resolvido por design** (context manager `__exit__`). 6 testes guard ephemeral + 1 realocação seed; deletado test_simulation_pool_warmup.py (12 testes obsoletos). v2.28: Warmups C+D (INEFICAZ — regressão persistiu). v2.27: 1 future ao invés de n_workers. v2.26: warmup como Future. v2.25: fix shape warmup. v2.24: Débitos Técnicos + I2.5 + I2.6. v2.23: Fastmath + Threads Adaptativos. v2.22.7-docs: pré-mortem + §74 + §75. v2.22.6: Fase 1 Fundação Multi-Agente. v2.22.4: `use_flat_prange=True` default. v2.21: causa-raiz KB-013 Cenário E 46k→122k. Paridade Fortran <1e-12 inviolável · 1660+ PASS / 295 SKIP / 0 FAIL na suite total (v2.29) |
