...
| **Simulation Manager** | **v2.27 (2026-05-10) — Fix warmup LLVM saturation (1 worker → cache compartilhado)**: Bug v2.26 — `PoolWarmupThread.run()` submetia `n_workers` futures simultâneos → saturação LLVM → 35 s/worker → **110+ s total**. Fix: 1 único future → 1 worker compila JIT e grava cache em disco; workers 2..N carregam bytecode do cache (< 2 s) via warmup secundário em `run_numba_chunk`. Segundo bug: `_WORKER_INITIALIZED` nunca setado após warmup secundário → rodava a cada chunk. 2 novos testes (total 10 warmup PASS). v2.26: warmup movido do inicializador para Future cancelável (elimina hang no shutdown). v2.25: fix shape warmup (`esp=[5.0,5.0]` → `esp` 8 elem com 10 camadas). v2.24: Débitos Técnicos + I2.5 + I2.6 (multi-agente). v2.23: Fastmath Dual-Mode + Threads Adaptativos. v2.22.7-docs: pré-mortem inaugural + §74 + §75 + §24.4. v2.22.6: Fase 1 Fundação Multi-Agente COMPLETA (§22.1). v2.22.4: `use_flat_prange=True` default. v2.21: causa-raiz KB-013 Cenário E 46k→122k. Paridade Fortran <1e-12 inviolável · 1667+ PASS / 295 SKIP / 0 FAIL na suite total (v2.27) |
