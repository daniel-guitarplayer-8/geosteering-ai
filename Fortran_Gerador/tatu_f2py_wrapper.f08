...
subroutine simulate(nf, freq, ntheta, theta, h1, tj, nTR, dTR, p_med, &
  ...
  do t = 0, maxthreads-1
    ...
    allocate(ws_pool(t)%den_TM_inv(npt, nf), ws_pool(t)%As_pos(npt, nf))
    allocate(ws_pool(t)%As_neg(npt, nf), ws_pool(t)%Bs_pos(npt, nf))
    allocate(ws_pool(t)%Bs_neg(npt, nf), ws_pool(t)%den_TE_inv(npt, nf))
    allocate(ws_pool(t)%Au_pos(npt, nf), ws_pool(t)%Au_neg(npt, nf))
    allocate(ws_pool(t)%Bu_pos(npt, nf), ws_pool(t)%Bu_neg(npt, nf))
    ws_pool(t)%last_camadT = -1
  end do

  ! Alocação dos caches Fase 4
  allocate(u_cache(npt, n, nf), s_cache(npt, n, nf))
  allocate(uh_cache(npt, n, nf), sh_cache(npt, n, nf))
  allocate(RTEdw_cache(npt, n, nf), RTEup_cache(npt, n, nf))
  allocate(RTMdw_cache(npt, n, nf), RTMup_cache(npt, n, nf))
