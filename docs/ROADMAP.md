...
| `A-jax-gpu-dispatcher` | A | Implementar `simulation.simulate(cfg, backend=...)` em `simulation/__init__.py` com `backend ∈ {"jax_gpu","numba_cpu","auto"}`; auto-detecta GPU via `jax.devices()`; ativa `jax_vmap_real=True` em GPU; adiciona `cfg.jax_cache_dir` em `config.py`; migra callers internos | 6-8h | P1 | BACKLOG | `A-jax-gpu-batched-api` + `A-jax-gpu-benchmark-redesign` |
