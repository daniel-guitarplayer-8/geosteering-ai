...
| `A-jax-gpu-data-gen` | A | Adicionar `cfg.surrogate_backend ∈ {"jax_gpu","numba_cpu","auto"}` em `config.py`; refatorar `data/surrogate_data.py` para usar `SyntheticDataGenerator` on-the-fly; chunking VRAM A100 (n_models > 500); fallback automático Numba se `jax.devices() == [cpu]` | 6-8h | P1 | **DONE-PARTIAL** (v2.46, 2026-05-30) | `SyntheticDataGenerator.generate_batch` agora usa o caminho **batched + agrupamento por geometria** (`simulate_multi_jax_batched_grouped`), multi-config (freq×dip×TR), `n_geometries` (geometrias batcháveis), `jax_chunk_size_models` (VRAM). Paridade <1e-13; cross-backend <1e-10. **Resta**: integração FV/GS + `DataPipeline`. Ver [docs/reports/v2.46_jax_datagen_batched_grouped_2026-05-30.md](reports/v2.46_jax_datagen_batched_grouped_2026-05-30.md) |
