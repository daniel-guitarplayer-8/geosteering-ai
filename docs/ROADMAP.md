...
| `C-surrogate-train` | C | F4.3 SurrogateNet Training (TCN 127M + ModernTCN 204M) em Colab Pro+ A100 com mp16 e dados gerados via JAX GPU on-the-fly | 4-8h GPU + 4-6h prep | P1 | **DONE-PARTIAL** (Sprint C, 2026-05-31) | **Fim-a-fim VALIDADO no A6000 local**: Fase A gera 30k via `simulate_batch("auto")`→jax (491s) → Fase B treina SurrogateNet (`build_surrogate`) com TF+XLA, **loss↓** (fim-a-fim OK). Achado: reassembly O(n) do grouped não escala a 30k → usar blocos homogêneos + concat (candidato a otimização). **Resta**: refatorar `data/surrogate_data.py` p/ on-the-fly + treino de convergência. Ver [docs/reports/v2.48_sprint_c_surrogate_train_e_o6_research_2026-05-31.md](reports/v2.48_sprint_c_surrogate_train_e_o6_research_2026-05-31.md) |
