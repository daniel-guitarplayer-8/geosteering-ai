...
| `A-jax-gpu-benchmark-redesign` | A | Rewrite notebook `validate_jax_gpu_v240.ipynb` para consumir API batched `simulate_multi_jax_batched`: warmup completo por cenário com dados reais (todos shapes e dips); `block_until_ready()` explícito antes de medir; cold-start (Run 1) reportado separadamente de hot-cache (Runs 2-5); baseline Numba medido localmente no T4 via `geosteering-cli benchmark` (não hardcoded de Intel i9-9980HK); `statistics.median` calculado excluindo Run 1 | 4-6h | P1 | CANDIDATE | `A-jax-gpu-batched-api` (DONE v2.42) |
