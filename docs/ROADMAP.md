...
| `A-jax-gpu-batched-api` | A | **[NOVO]** Implementar API batched `simulate_multi_jax(models: list[dict])` via `jax.vmap` sobre eixo `n_models`; elimina loop Python serial em `multi_forward.py:400-422`; remove `build_static_context()` por modelo (5-20 ms overhead); elimina `np.asarray(H_jax)` síncrono por modelo (GPU→CPU sync); usa `_UNIFIED_JIT_CACHE` keyed `(n, npt)` invariante a valores de modelo | 8-12h | P1 | CANDIDATE | `A-jax-gpu-validate` (DONE-PARTIAL) |
| `A-jax-gpu-benchmark-redesign` | A | **[NOVO]** Rewrite notebook `validate_jax_gpu_v240.ipynb`: warmup completo por cenário com dados reais (todos shapes e dips); `block_until_ready()` explícito antes de medir; cold-start (Run 1) reportado separadamente de hot-cache (Runs 2-5); baseline Numba medido localmente no T4 via `geosteering-cli benchmark` (não hardcoded de M-series); `statistics.median` calculado excluindo Run 1 | 4-6h | P1 | BACKLOG | `A-jax-gpu-batched-api` |
