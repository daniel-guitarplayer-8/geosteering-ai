...
|:-------:|:-----------|:-------------------------------------------------------------|:-----------:|:-------------------------------------|
| **v2.56** | **2026-06-02** | **`geosteering-cli`: wall-clock JAX < Numba + transparência de tempo** ✅: causa-raiz (workflow 6-agentes + bissecção A6000 + verificação adversarial — **FALSIFICOU a hipótese de cache XLA**) — (1) discrepância "tempo 2,14 s vs `time real` 25,1 s" = a tabela só mostrava `median(hot)` (escondia startup+warmup) e `elapsed_s = last_elapsed` ≠ `throughput = median`; (2) JAX wall-clock (25,1 s) > Numba (19,5 s) apesar de ~2× throughput NÃO é compilação (cache XLA cheio: 284/284 hits, 2º run idêntico) e sim **tracing Python por-grupo** sobre os 31 grupos `K=n//32`, re-pago a cada processo. **Fix (validado A6000)**: default `templates` de **poucos grupos grandes** (`sample_geometry` K: `n//32` → `max(1, min(n//256, 4))`, n=1000: 31→3) corta o tracing + satura a GPU; skip `warmup_backend` morto no jax (shape `n_models=1`); `t_warmup`/`t_total` + `elapsed_s=median` + rows "tempo de warmup"/"tempo total" na tabela; novo **`--jax-chunk-size`** + `resolve_jax_chunk_size` (auto-chunk 64 anti-OOM em high-config G/H). **Investigado (pedido do usuário)**: persistência de warmup já maximal nos 2 backends; **`jax.export` (AOT) MEDIDO inefetivo** (`deserialize+call` 0,093 s ≈ `trace+compile-hit` 0,094 s) — o lever é reduzir K, não persistir. **Medido: JAX warm `real` 12,75 s < Numba 19,97 s; hot 1,84M mod/h (+13%, ±10k vs ±143k); G/H auto-chunk sem OOM; Numba E n=200 = 234.536 (sem regressão); Paridade Fortran 10/10**. Ver [report v2.56](reports/v2.56_cli_jax_wallclock_warmup_2026-06-02.md) | +7 (test_cli_timing) | `feat/jax-perf-o2-o3` |
| **v2.55** | **2026-06-02** | **`geosteering-cli`: crash TLS do JAX + lentidão (cold-start)** ✅: causa-raiz (workflow 7-agentes + bissecção + verificação adversarial, reproduzido 3/3) — (1) CRASH `_dl_allocate_tls_init` = exaustão de **TLS estático IN-PROCESS**: `resolve_backend("jax")`→`jax.devices()` inicializa CUDA no PAI (consome surplus TLS) e o warmup JIT `models[:1]` (1 modelo → não-agrupável) cai p/ Numba → pool **libgomp 64 threads** estoura; (2) "JAX 5.5× mais lento" = **ARTEFATO DE COLD-START** (warmup compilava ZERO programas JAX). **Fix híbrido (validado A6000)**: pré-voo de geometria jax-free (`_count_geometry_groups`/`resolve_backend_preflight`) → jax+não-agrupável roda Numba **SEM init CUDA** (sem crash); warmup JIT jax usa o **workload completo** (aquece XLA → timed-run quente); teto `NUMBA_NUM_THREADS=cpu//2`/`OMP=1`/`OPENBLAS=1` no startup (defesa TLS); log auto-detect `info→debug`. **Medido: crash per-model 3/3→OK; templates n=500 warm = 1.66M mod/h (~2× Numba 840k); Numba E n=200 = 239k (sem regressão); Paridade Fortran 10/10**. Ver [report v2.55](reports/v2.55_cli_jax_crash_tls_fix_2026-06-02.md) | +9 (test_cli_jax_crash_guard) | `feat/jax-perf-o2-o3` |
| **v2.54** | **2026-06-02** | **`geosteering-cli`: correção da lentidão/trava do backend JAX** ✅: causa-raiz (workflow 7-agentes + verificação adversarial) — modelos com geometria (`esp`) ÚNICA por modelo → N grupos → JAX-grouped degenerava em N chamadas de 1 modelo (zero batching, travava) + `run_once` forçava `numba_fallback=False` (desligava a proteção do dispatcher). **Fix híbrido**: `--backend jax` não-agrupável cai p/ Numba (sem travar; backend EFETIVO + `motivo` na tabela); novo **`--geometry {per-model,templates,quantized}`** (`cli/_exec.py::sample_geometry`, espelha `synthetic_generator`) → `templates` satura a GPU (medido `backend=jax`, 2 grupos). **`--compare-backends` continua forçando JAX** (FURO 1 — paridade real medida = **3,86e-14**, não 0). Silencia log Numba auto-detect no JAX; guard warning-only no grouped (não reroteia, FURO 3). **Numba INTOCADO (E n=200 = 227.727 mod/h ≈ baseline); Paridade Fortran 10/10**. Ver [report v2.54](reports/v2.54_cli_jax_slowness_fix_2026-06-02.md) | +17 (13 novos + 4 backend) | `feat/jax-perf-o2-o3` |
| **v2.53** | **2026-06-02** | **`geosteering-cli`: backend JAX selecionável + tabela ASCII + salvar `.dat`/`.out`** ✅: `--backend {numba,jax}` (padrão numba CPU / jax GPU via dispatcher `simulate_batch`, paridade `max\|Δ\|=2,71e-14`); **tabela ASCII** de resultados (`cli/_table.py` — throughput, tempo, workers×threads/strategy, hardware via `cli/_hwinfo.py`, NaN/Inf); **`.dat`/`.out` 22-col** conforme `geosteering-physics.md` §4 (`res_h`/`res_v` REAIS na camada via `_layer_at_batch`; escritores relocados p/ `simulation/io/tensor_dat.py`); recursos `--json`/`--repeat`/`--list-scenarios`/`--compare-backends`; fix bug `.npz` (`repr`→`H_stack`). **Numba byte-a-byte legado (zero regressão: E n=200 = 233.692 mod/h ≈ 234.665); Paridade Fortran 10/10**. Ver [report v2.53](reports/v2.53_cli_backend_jax_tabela_dat_2026-06-02.md) | +31 (28 novos + 3 mvp) | `feat/jax-perf-o2-o3` |
| **v2.40** | **2026-05-18** | **I2.2 MCP colab-bridge + tf.data + Mixed Precision** ✅: Skill `geosteering-colab-mcp` (Sonnet 4.6) + hook `colab-token-refresh.sh` + 3 templates `notebooks/colab_templates/` (train mp16+XLA, validate JAX GPU 109 tests, benchmark tf.data 4 configs × 5 runs). **F4.3 SurrogateNet desbloqueado** (treinamento remoto Colab Pro+ A100 com mp16). **Fix D5**: bug ordem `setup_mixed_precision_policy()` agora módulo-level chamado ANTES de `build_model()` (era depois — camadas fp32 mesmo com flag True). Helper `build_model_with_mp_policy(config)` em `models/registry.py`. **+4 flags `PipelineConfig` D6**: `tf_shuffle_buffer_size`, `tf_num_parallel_calls`, `tf_prefetch_buffer_size`, `tf_cache_eval` (substituem hardcodes em `build_tf_dataset`). **+pytest marker `gpu`**: 109 testes JAX skipam em CPU, executam em Colab T4. Resposta D3 à pergunta arquitetural: SIM, automação JAX GPU via caminhos (a) template Colab + (c) parcial pytest marker. Endpoint `/simulate` adiado v2.41+. | +16 novos (12 tf.data + 4 mp16) | `feature/v2.40-colab-bridge-mp16` |
...
| **v2.35** | **2026-05-15** | **Cenário H (estresse multi-core 8×8×8 = 512 combos)** no benchmark CLI: 8 frequências log-spaced (1 kHz–200 kHz) × 8 espaçamentos TR (0.25–2.5 m) × 8 dips (0°–105°). Stress-test para CPUs multi-core, complementa A–G. 3 novos testes (help text, dict shape, smoke `--n 2 --workers 2 --threads 2` em <600s). Sem mudança no caminho crítico Numba JIT — paridade Fortran <1e-12 preservada. | 3 novos | `main` (em progresso) |
| **v2.34** | **2026-05-15** | **`geosteering-warmup` integrado no CI/CD**: workflow GitHub Actions adiciona steps `geosteering-warmup --verbose` + `geosteering-cli benchmark --scenario E --n 200` para isolar cold-start JIT do tempo medido. Nova métrica `E_n200_warm` em `.claude/perf_baseline.json` + seção "Warm-Cache Baseline (CI)" em `docs/PERFORMANCE_BASELINE.md`. `continue-on-error: true` no benchmark (observabilidade, não gating). | 0 novos (CI only) | `main` (em progresso) |
| **v2.33** | **2026-05-15** | **Suite pytest-qt para GUI Simulation Manager**: cobertura automatizada de ~10.7k linhas Qt (antes: zero testes GUI). `tests/conftest.py` raiz alinha QT_API com sm_qt_compat (evita cross-binding TypeError). `tests/conftest_qt.py` expõe fixtures `qt_binding` + `mock_simulation_thread` + `mock_sim_request`. `tests/test_simulation_manager_gui.py` cobre 16 testes em 4 grupos: compat Qt, SimRequest dataclass, SimulationThread (sinais + pause/resume cooperativo), widgets leves (WelcomeWidget, ParametersPage roundtrip). CI: `xvfb-run -a pytest` headless em Ubuntu runner. `pytest-qt>=4.4` adicionado a `[dev]`. Marker `gui` adicionado. | 16 novos GUI | `main` (em progresso) |
...
|:--|:--:|:--|:--:|:--:|:--:|:--|
| `A-jax-gpu-validate` | A | Validar `validate_jax_gpu_v240.ipynb` em T4 + A100 real; medir cenários A–H × `{vmap, vmap_real}` × 5 runs; publicar seção "JAX GPU Backend Baseline" em `docs/PERFORMANCE_BASELINE.md`; confirmar paridade Fortran <1e-12 em GPU | 3-4h Colab | P1 | DONE-PARTIAL | Paridade OK (163/163 pytest PASS); gate performance falhou (A: 0.38×, B: 0.37×, E: 0.61× Numba — abaixo de 1.5×). Causa-raiz: ausência de API batched + loop Python serial. Ver `docs/perf_baselines/sprint_a1_jax_benchmark_t4_20260519_192245.json` e `docs/reports/v2.40.4_auditoria_resultados_sprint_a1_2026-05-19.md` |
| `A-jax-gpu-batched-api` | A | Implementar API batched `simulate_multi_jax_batched(rho_h_batch, rho_v_batch, esp_batch, ...)` via `jax.vmap` sobre eixo `n_models`; elimina overhead Python (5-20 ms/modelo); single `block_until_ready` + `np.asarray`; reutiliza `_UNIFIED_JIT_CACHE` keyed `(n, npt)` invariante a valores de modelo | 8-12h | P1 | **DONE** (v2.42, 2026-05-20) | `A-jax-gpu-validate` (DONE-PARTIAL). Paridade max \|diff\| = 8.33e-14 (24/25 testes PASS). Ver [docs/sprints/v2.42.md](sprints/v2.42.md) |
| `A-jax-gpu-benchmark-redesign` | A | Rewrite notebook `validate_jax_gpu_v240.ipynb` para consumir API batched `simulate_multi_jax_batched`: warmup por-cenário com batch size exato (XLA cache key inclui n_models); cold-start (Run 1) reportado separadamente de hot-cache (Runs 2-5); baseline Numba medido localmente no T4 via `geosteering-cli benchmark` (não hardcoded de Intel i9-9980HK); `statistics.median` excluindo Run 1; 8 bugs metodológicos do v2.40 corrigidos (C1+C2+H1-H3+M1-M3) | 4-6h | P1 | **DONE** (v2.43, 2026-05-22) | `A-jax-gpu-batched-api` (DONE v2.42). Ver [docs/sprints/v2.43.md](sprints/v2.43.md). Execução em Colab T4 = deliverable do usuário |
| `A-jax-gpu-dispatcher` | A | Implementar `simulation.simulate(cfg, backend=...)` em `simulation/__init__.py` com `backend ∈ {"jax_gpu","numba_cpu","auto"}`; auto-detecta GPU via `jax.devices()`; ativa `jax_vmap_real=True` em GPU; adiciona `cfg.jax_cache_dir` em `config.py`; migra callers internos | 6-8h | P1 | **DONE** (Sprint B, v2.48, 2026-05-31) | `simulation.simulate_batch(..., backend="auto")` (`dispatch.py`) codifica a **árvore de decisão medida**: GPU + n≥32 + agrupável → JAX bucketed; senão → Numba 16w×4t. Auto-detect via `jax.devices()`; warn n<32; **guard anti-unified** (high-config OOM → ValueError). `"auto"` em `PipelineConfig`. `generate_batch` refatorado (DRY). **NOTA**: `jax_vmap_real` foi REFUTADO (regressão 0.60–0.74×, v2.45) — o dispatcher usa bucketed/grouped, nunca unified. Overhead +0.3%. 138/138 + 53/53 PASS. Ver [docs/reports/v2.48_sprint_b_dispatcher_2026-05-31.md](reports/v2.48_sprint_b_dispatcher_2026-05-31.md) |
| `A-jax-gpu-data-gen` | A | Adicionar `cfg.surrogate_backend ∈ {"jax_gpu","numba_cpu","auto"}` em `config.py`; refatorar `data/surrogate_data.py` para usar `SyntheticDataGenerator` on-the-fly; chunking VRAM A100 (n_models > 500); fallback automático Numba se `jax.devices() == [cpu]` | 6-8h | P1 | **DONE** (Sprint A, v2.47, 2026-05-30) | `generate_batch` usa o caminho **batched + agrupamento por geometria** com 3 modos (`geometry_mode` = templates/quantize/per_model) + `numba_fallback` automático (geologia não-agrupável) + `group_by_geometry` helper. Extração de features **FV/GS** via `to_feature_dataset`. Gate de performance `DG` (**1.89× Numba** @ 18cfg/600pos/n=64). Paridade <1e-13; 30/30 + 53/53 PASS. Ver [docs/reports/v2.47_sprint_a_datagen_geometry_grouping_2026-05-30.md](reports/v2.47_sprint_a_datagen_geometry_grouping_2026-05-30.md) |
| `C-surrogate-train` | C | F4.3 SurrogateNet Training (TCN 127M + ModernTCN 204M) em Colab Pro+ A100 com mp16 e dados gerados via JAX GPU on-the-fly | 4-8h GPU + 4-6h prep | P1 | **DONE-PARTIAL** (Sprint C, 2026-05-31) | **Fim-a-fim VALIDADO no A6000 local**: Fase A gera 30k via `simulate_batch("auto")`→jax (491s) → Fase B treina SurrogateNet (`build_surrogate`) com TF+XLA, **loss↓** (fim-a-fim OK). Achado: reassembly do grouped → **RESOLVIDO em v2.49** (scatter vetorizado: bit-exato Δ=0.0, **pico de memória −49 %**; o gargalo de escala real é o nº de geometrias, não a reassembly). **`data/surrogate_data.py` on-the-fly → DONE em v2.50** (`surrogate_pairs_from_batch` + `generate_surrogate_dataset` + `iter_surrogate_batches`; `dat_22col` vetorizado; 14 testes; DG gate 6/6; benchmark produtizado). **Resta**: apenas o **treino de convergência** (GPU-hours) usando `iter_surrogate_batches`. Ver [v2.48 Sprint C](reports/v2.48_sprint_c_surrogate_train_e_o6_research_2026-05-31.md) + [v2.49 reassembly+O6](reports/v2.49_reassembly_vetorizado_e_pesquisa_levers_o6_2026-05-31.md) + [v2.50 on-the-fly API](reports/v2.50_surrogate_onthefly_data_api_2026-06-01.md) |
...
| `F-mvc-split` | F | MVC split físico do Simulation Manager → **SM MVVM (Strangler Fig)** | multi-sprint | P1 | **IN_PROGRESS** (v2.57) | Fundação `gui/` + SM MVVM Fatias 0-5 DONE (specs 0004-0012, mergeadas v2.57). Em curso: 0013 shell Antigravity, 0014 Fatia 6a (execução), 0015 Fatia 6b (geologia). Paridade total = Fatias 6c-6i (Resultados/Benchmark/Análise/Preferências). Ver `docs/reports/sm_mvvm_investigacao_paridade_design_sdd_premortem_2026-06-06.md` |

### Specs GUI/SM MVVM completadas (v2.57, 2026-06-06)

| Spec(s) | Entrega | Released-As |
|:--|:--|:--:|
| 0004-0007 | Fundação `geosteering_ai/gui/` (qt_compat, MVVM base, plot backends, persistência) | v2.57 |
| 0011, 0011a-d | SM MVVM Fatias 0-4 (de-shim → skeleton → params → geologia estocástica → galeria) | v2.57 |
| 0012 | JAX-GPU no SM MVVM (subprocesso spawn TLS-safe; paridade <1e-12) | v2.57 |
| 0013-0015 | **Em andamento**: shell Antigravity + Fatia 6a (execução) + Fatia 6b (geologia avançada) | — |
