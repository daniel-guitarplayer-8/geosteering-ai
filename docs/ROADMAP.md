...
| `C-surrogate-train` | C | F4.3 SurrogateNet Training (TCN 127M + ModernTCN 204M) em Colab Pro+ A100 com mp16 e dados gerados via JAX GPU on-the-fly | 4-8h GPU + 4-6h prep | P1 | BLOCKED | `A-jax-gpu-batched-api` + `A-jax-gpu-benchmark-redesign` + `A-jax-gpu-dispatcher` + `A-jax-gpu-data-gen` |
