...
| `A-jax-gpu-data-gen` | A | Adicionar `cfg.surrogate_backend ∈ {"jax_gpu","numba_cpu","auto"}` em `config.py`; refatorar `data/surrogate_data.py` para usar `SyntheticDataGenerator` on-the-fly; chunking VRAM A100 (n_models > 500); fallback automático Numba se `jax.devices() == [cpu]` | 6-8h | P1 | BACKLOG | `A-jax-gpu-dispatcher` |
