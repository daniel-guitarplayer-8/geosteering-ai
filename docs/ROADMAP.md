...
| **v2.19** | **2026-05-02** | **Fix bug funcional do gerador aleatório (`rng_seed=42` hardcoded) + UI control de semente (default aleatório) + `nogil=True` universal no hot path Numba (`_simulate_positions_njit{,_cached}`) + benchmark CLI CPU-aware defaults via `recommend_default_parallelism()` + warning de oversubscrição. Cenário A: 189k→802k mod/h (4×) com defaults corretos. Auditoria PyQt6 descarta como causa-raiz da regressão histórica.** | **12 novos (7 random seed + 5 pool warmup não-regressão)** | **`feat/simulation-manager-v2.19`** |
