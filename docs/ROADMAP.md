...
| **v2.23** | **2026-05-10** | **Fastmath Dual-Mode + Threads Adaptativos** (multi-agente Hub-and-Spoke): A.1 `@njit(fastmath=True)` em `_numba/geometry.py` (3 funções) + `_numba/rotation.py` (2 funções) — paridade Fortran <1e-12 PRESERVADA em 13/13 modelos canônicos (oklahoma_3, 5, 28, hou_7, devine_8, oklahoma_15, viking_graben_10). A.2 auto-detect via `recommend_default_parallelism()` no `__post_init__` quando `n_workers=None AND threads_per_worker=None` — backward-compat preservada (n_workers=1 explícito = single-process). Defesa em camadas KB-019: warning se `n_workers × threads > 4× physical_cores`. Tolerante a falhas: try-except no lazy import. Hook `setup-environment.sh` exibe topologia + defaults v2.23. Skill v2.23: `geosteering-physics-reviewer` (Opus 4.7) + `geosteering-perf-reviewer` (Haiku 4.5) + `geosteering-code-reviewer` (Sonnet 4.6) em paralelo. | 7 novos (v2.23) + 1 atualizado (test_defaults_*) | `feat/sprint-v2.23-fastmath-adaptive-threads` |
| **v2.24** | **2026-05-10** | **Débitos Técnicos + I2.5 + I2.6** (multi-agente): **Frente 1**: 1.1 `scripts/count_pytest_pass.py` automatiza contagem PASS (M2 recorrente fechado). 1.2 `setup-environment.sh` totalmente em PT-BR acentuado (11+ ocorrências). 1.3 `cache=True` explícito em TODOS os 12 decoradores `@njit` em `_numba/` (propagation, dipoles, rotation, hankel, geometry). 1.4 `use_fastmath` documentado como status documental (dispatcher real deferido v2.25+). **I2.5**: hooks `check-ptbr-accentuation.sh` (catálogo `.claude/ptbr-words.txt` 60+ pares, registrado em settings.json PostToolUse) + `generate-pr-description.sh` (template `.claude/templates/pr_description_template.md`). **I2.6**: CLI MVP `geosteering-cli` (subcomandos `simulate`/`benchmark`/`version`) em `geosteering_ai/cli/` + entry point `pyproject.toml`. Lazy imports nos handlers para `--help` rápido. Reutiliza `simulate_multi` + `recommend_default_parallelism`. Paridade Fortran <1e-12 PRESERVADA em 13/13 modelos. | 41 novos (11 v2.24 + 17 I2.5 + 13 CLI) | `feat/sprint-v2.24-debits-i25-i26` |
